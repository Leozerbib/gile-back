...
export class WorkspaceGatewayController {
  ...
  async create(@CurrentUser() user: any, @Body() body: CreateWorkspaceDto): Promise<WorkspaceDto> {
    const result = await this.workspaces.create(user.id, body);
  ...
  @ApiOkResponse({ type: WorkspaceOverview, isArray: true })
  async getMyWorkspaces(@CurrentUser() user: AuthenticatedUser): Promise<WorkspaceOverview[]> {
    const result = await this.workspaces.getOverview(user.userId, { skip: 0, take: 50 });
    return normalizeObject(result.items ?? []) as WorkspaceOverview[];
  }
  ...
  async get(@CurrentUser() user: AuthenticatedUser, @Param("id") id: string): Promise<WorkspaceDto> {
    const result = await this.workspaces.findById(id, user?.userId);
    return normalizeObject(result) as WorkspaceDto;
  }
  ...
  async update(@CurrentUser() user: AuthenticatedUser, @Param("id") id: string, @Body() body: UpdateWorkspaceDto): Promise<WorkspaceDto> {
    const result = await this.workspaces.update(id, body, user.userId);
    return normalizeObject(result) as WorkspaceDto;
  }
  ...
  async remove(@CurrentUser() user: AuthenticatedUser, @Param("id") id: string): Promise<{ success: boolean }> {
    return normalizeObject(await this.workspaces.remove(id, user?.userId));
  }
}
