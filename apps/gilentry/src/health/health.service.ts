      ...
      services.map(async service => {
        try {
            ...
            service.health.check({ service: service.name }).pipe(
              ...
              catchError(err => {
                void this.logger.log({
                  ...
                  data: { error: String((err as Error)?.message ?? err), service: service.name },
                });
          ...
          void this.logger.log({
            ...
            data: { error: String((error as Error)?.message ?? error), service: service.name },
          });
  ...
  async checkService(serviceName: string) {
    ...
    try {
        ...
        healthService.check({ service: serviceName }).pipe(
          ...
          catchError(err => {
            void this.logger.log({
              ...
              message: "Microservice unavailable",
              data: { error: String((err as Error)?.message ?? err), service: serviceName },
            });
            return of<HealthResponse>({ status: "unavailable" });
          }),
        ),
      ...
      void this.logger.log({
        ...
        func: `health.check:${serviceName}`,
        message: "Microservice unavailable (exception)",
        data: { error: String((error as Error)?.message ?? error), service: serviceName },
      });
